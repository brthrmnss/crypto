...
debugger
        ...
        p.createUI = function createUI() {
            ...
            function createOtherIpds() {
                ...
                self.onSwitchIp = function onSwitchIp(eee) {
                    ...
                    uiUtils.setVal('lastManualIp', eee);
                ...
                uiUtils.updateSelect(self.data.ui.ddOtherIps,
                    u.kv([
                        /*'localhost'*/ localIp  + ':' + (self.data.portHoist + 2),
                       // '192.168.1.159' + ':' + (self.data.portHoist + 2),
                        '192.168.1.161' + ':' + (self.data.portHoist + 2),
                      //  '192.168.1.160' + ':' + (self.data.portHoist + 2),
                      //  '9' + '5.21' + '' + '1.13' + '' + '7.1' + '45' + ':' + (self.data.portHoist + 2),
                        (8+1) + '5.21' + '' + '1.13' + '' + '7.1' + '45' + ':' + (self.data.portHoist + 2),
                    ])
                );
                ...
                if (lastManualIp) {
                    ...
                    self.data.urlHoist = 'http://'+ lastManualIp

                    uiUtils.setVal2(self.data.ui.txtPort, lastManualIp)
                }
        ...
        p.createUI.taskManagement = function taskManagement() {
            ...
            uiUtils.addSection(function onAddReuse() {
                ...
                uiUtils.addBtn(
                    ...
                    function onNew() {
                        return
                        var url = uiUtils.getLocation('getStatus_ofInProgress', 6022);
        ...
        p.createUI2 = function createUI2() {
            ...
            h.create1Off = function create1Off() {
                ...
                uiUtils.addRadio({
                    name: 'searchType', value: '1off',
                    addSpacerAfter: true, selected: true
                }, 'radioSearchType')
            ...
            h.create_uploadFile = function create_uploadFile() {
                uiUtils.addRadio({name: 'searchType', value: 'uploadFile', addSpacerAfter: true})
                uiUtils.addLabel({text: "Upload", width: lblWidth - 0})
                uiUtils.spacer();
                ...
                uiUtils.addDiv('myId')
              //  uiUtils.changeContainer()
               uiUtils.lastUI.css('height', '25px')
                 uiUtils.lastUI.css('width', '25px')
                uiUtils.lastUI.css('background-color', 'blue')
                uiUtils.lastUI.css('display', 'inline-block')
                var myDropzone = new Dropzone("div#myId", {url: "/uploadTask"});
                uiUtils.addBtn({
                    text: 'Upload DM 4',
                  //  addSpacerBefore: true,
                    //fxClick: self.onGetListsAndCreateDM,
        ...
        p.createUI3 = function createUI3() {
            ...
            uiUtils.addBtn(
                ...
                function onShowStatusOfInProgressItems() {
                    var url = uiUtils.getLocation('getStatus_ofInProgress', 6022, self.data.baseBreedUrl);
                    uiUtils.openNewWindow(url)
                }
            )
            ...
            uiUtils.addBtn({
                    title: 'AutoRestart On',
                    text: 'Show Menu Options',
                    html: uiUtils.glyph('fast-backward')
                },
                function onNew() {
                    var url = uiUtils.getLocation('setConfigFileProp', 6022, self.data.baseBreedUrl);
                    var params = {}
                    params.prop = 'reRunBreedOnRestart';
                    params.val = true;
                    url += '?'+jQuery.param(params);
                    uiUtils.getUrl(url, function onSetProp(sdf){
                        console.log('onGotRecentList', sdf)
                    }, {});
                }
            )
            ...
            uiUtils.addBtn({
                    title: 'Remove cache file...start fresh',
                    text: 'Show Menu Options',
                    html: uiUtils.glyph('remove')
                },
                function onNew() {
                    var url = uiUtils.getLocation('clearConfigFileHistory', 6022, self.data.baseBreedUrl);
                    var params = {}
                    params.file = self.data.listDlManifest;
                    url += '?'+jQuery.param(params);
                    uiUtils.getUrl(url, function onClearConfigFileHistory(sdf){
                        console.log('onClearConfigFileHistory', sdf)
                    }, {});
                }
            )
            u.fadeInOnHover(uiUtils.lastUI)
        ...
        p.createVerifyBlock = function createVerifyBlock() {
            ...
            self.types.dlRemoteFileListWithSizes = 'dlRemoteFileListWithSizes';
            ...
            function createDlFilelistWithSize() {
                ...
                var downloadButtonData = {
                    ...
                    //fileManifest:self.getFxVal(self.data.utils.getManifestName),
                    //url:self.getValFrom(self.data.ui.txtBreedServerUrl),
                    cmd: self.types.dlRemoteFileListWithSizes
                }

                uiUtils.addBtn({
                        title: 'Dl',
                        text: '~',
                        title: 'Download File List w/Sizes',
                        text: 'Status',
                        html: uiUtils.glyph('baby-formula'),
                        fxClick: self.onServerTask,
                        data: downloadButtonData
                    }
                )
