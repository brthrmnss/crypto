...
function Morpher() {
    ...
    p.init = function init(cfg) {
        ...
        sh.requireJquery(cfg.div, 'could not find the partial', 10)
        sh.require(cfg.div, 'need idPartial id')

    }

    p.render = function render(query) {
        sh.cid(self.settings.fxRender)
        var div = $(self.settings.div);
        var cfg = {}
        //cfg.div = '.modal-contact .content'
        // cfg.append = true
        // cfg.divCreatable = u.join2('holder', self.settings.id)
        cfg.id = self.settings.div;
        cfg.id = cfg.id.replace('#', '')
        //cfg.url = "/themes/minimal_v0" + "/js/comps/simpleList.html"
        cfg.url = '/grid/grid/sharedResourcesGrid/' + 'comps/user/MorpherTestDom.html'
        //console.log('log', '...', document.currentScript)
        //debugger
        cfg.fxDone = self.processPage;
    ...
    p.processPage = function processPage(query) {
        console.debug()
