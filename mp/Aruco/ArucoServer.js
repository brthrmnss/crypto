...
function RCConfigExecServer() {
    ...
    self.runServer = function runServer() {
       ...
       // app.use(sh.blockIndexPage(indexPageSecurityEnding));
        ...
        var dirBaby = sh.fs.join(__dirname, '..', 'BabylonJS/public_html' )
        app.use(express.static(dirBaby));
        /*return;
        ...
        app.post('/uploadPic', function uploadPic (req, res) {
            var body = req.body;
            var name = body.name;
            var contents = body.uploadPic;
            //var name = req.params.name;
            var dirUploads = sh.fs.join('public_html2', 'upload/')
            sh.fs.mkdirp(dirUploads)
            console.log('j', contents.length)
            if ( name == null ) {
                name = sh.getTimeStamp()
            }
            sh.writeFile(dirUploads+'/'+name+'.png',
                contents, false, 'base64')
        ...
        app.get('/listPics', function onSaveFile (req, res) {


            var body = req.body;
            var name = body.name;
            var contents = body.body;
            var dir = sh.fs.join(__dirname, 'public_html2', 'upload')
            //var name = req.params.name;
            var files = sh.fs.getFilesInDirectory2(dir)

            sh.each.replace = function replace(arr, replaceThis, withThis) {
                var files2 = [];
                sh.each(arr, function copyC(k,v){
                    v = v.replace(replaceThis, withThis)
                    files2.push(v)
                })
                return files2
            }
            files = sh.each.replace(files, dir+'/', '/upload/')
            console.log('ok', 'list', files)
            res.json(files);///('Hello World!');
        });
