...
#why: enable user tos cript operation of box
...
class testHTTPServer_RequestHandler(BaseHTTPRequestHandler):
  ...
  def do_POST(self):
        ...
        post_body = unquote(unquote(post_body))
        #import json
        #import urlparse
        #post_body = urlparse.parse_qs(post_body)
        prefix = "data="
        if post_body.startswith(prefix):
            post_body = post_body[len(prefix):]
        ...
        #post_body = "y = 5 + 2"
        out = execute(post_body, globs, locs)

        #print "out:", s.getvalue()
        print("post_body_result:")
        print(out);
