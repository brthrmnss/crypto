...
function start(port_secure, port_insecure, fxApp ) {


  var port = process.argv[2] || port_secure;
  var insecurePort = process.argv[3] || port_insecure;
  var fs = require('fs');
  var path = require('path');
...
//
  ...
  checkip.getExternalIp().then(function (ip) {
    ...
    var app = fxApp(server, host, port, publicDir);
    listen(app);
  });
...
exports.expressHttpsHelper = start;