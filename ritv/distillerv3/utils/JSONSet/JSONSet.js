...
function JSONSet() {
    ...
    p.getFiles = function getFiles(config) {
        ...
        if (  self.settings.includeAllItmes == true ) {
            self.data.work.list = [];
            self.proc('include all items')
        }
        //self.data.work.list = []; //why: force refresh
        if ( self.data.work.list.length == 0 ) {
            self.proc('list is emtpy, so grabbing from input file', self.data.fileInput )
        ...
        /*
        if ( self.settings.fieldDict ) {
            var dict = sh.makeDict(self.settings.fieldDict, self.data.work.list)
        }
    ...
    p.finishedWriting = function finishedWriting() {
        ...
        //asdf.g
        self.proc('...', 'finished', self.data.fileOutput)
	  if ( self.settings.announce != false ) {
            self.proc('...', 'finished')
        }

        var saveOutput = true;
        if ( self.settings.includeAllItmes ){
            saveOutput = false;
        }
    ...
    function defineUtils() {
        ...
        u2.percentageOfWorkList =
            function percentageOfWorkList(arr, extratxt, lookFor) {
                extratxt = sh.dv(extratxt, '')
                //console.error(extratxt, 't')

                var percentage = sh.percent(
                    arr.length/
                    self.data.work.list.length
                )
