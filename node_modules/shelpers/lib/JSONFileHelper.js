...
function JSONFileHelper() {
    ...
    p.get = function getByProp(name, propMatch) {
        ...
        var match = null;
        if (self.settings.dictMode) {
            var key = name[propMatch]
            if (self.settings.fxGenerateKey) {
                key = self.settings.fxGenerateKey(name)
            }
            sh.throwIfNull(key, 'need a key for', name)
            if (self.settings.fxKeyTransform) {
                if (key == null) {
                    console.log('error', key, name)
                }
                key = self.settings.fxKeyTransform(key)
            }
    ...
    p.getKey = function getK(objForKey) {
        return self.get(objForKey)
    }
    p.setKey = function setKey(objForKey, saveIt) {
        return self.addRecent(objForKey, true, null, false);
    }


    p.addRecent = function addRecent(data, upsert, propMatch, saveFile) {
        propMatch = sh.dv(propMatch, self.settings.propUpsert);
        if (self.settings.dictMode) {
            var key = data[propMatch]
            if (self.settings.fxGenerateKey) {
                key = self.settings.fxGenerateKey(data)
            }
            sh.throwIfNull(key, 'need a key for', data)
            if (self.settings.fxKeyTransform) {
                key = self.settings.fxKeyTransform(key)
            }
            self.data.file[key] = data
            if (saveFile != false)
                self.saveFile()
            return data;
        }
    ...
    p.readFile = function readFile() {
        if (self.settings.dictMode != true) {
