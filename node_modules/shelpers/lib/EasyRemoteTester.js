...
function EasyRemoteTester() {
        ...
        self.quickRequest = function quickRequest(url, method, fx, postData, doRegPost, postJSON) {
            ...
            if (method == types.POST.toLowerCase()) {
                if (doRegPost == 'fileupload') {
                    ...
                    var form = req.form();
                    var field = sh.dv(reqoptions.uploadField, 'file')
                    form.append(field, fs.createReadStream(postJSON))
                    if (postData) {
    ...
    function defineUtils2() {
        ...
        self.createHelpers = function createHelpers() {
            ...
            data.assert = function assert(eq, msg) {
               ...
               if ( eq == false && throwError ) {
                    setTimeout(function clearStout() {
                       sh.x('quick escape')
                    }, 50)
               }
               if ( self.workChain.currentOperation == null ) {
                   console.error('current operation is null')
                   self.workChain.currentOperation = {};
               }
         ...
         */
        self.cb = function cb_callCurrentCallback(ooo) {
            self.data.lastStackPrex = new Error()
            self.workChain.data.lastStackPrex = new Error()
         //  sh.showLine('this is it')
            var delayRunning = false
            if ( delayRunning ) {
                setTimeout(function onStartLAterToPreventPromiseCatchingErrors() {
                     self.workChain.cb(); //break sequlize try catch chains
                }, 20)
            } else {
                //self.workChain.cb();
                self.workChain['cb'](ooo);//
            }
    ...
    function defineTestVerbHelpers() {
        ...
        self.getRNextUrl = function getRNextUrl(setFxToGetUrl) {
            self.fxNextUrlGetFx = setFxToGetUrl
        }
        self.getR = function getR(url, type) {
            ...
            if ( self.fxNextUrlGetFx ) {
                url  = self.fxNextUrlGetFx;
                self.fxNextUrlGetFx   = null;
            }
            ...
            function nextFunctionToAdd_getR() {
                ...
                if (innerDataForFxCall.uploadPath != null) {
                    ...
                    reqObj.uploadField = innerDataForFxCall.uploadField;
                }
                ...
                reqObj.url = url;
                nextFunctionToAdd_getR.name = nextFunctionToAdd_getR+' '+url
                ...
                function onResult(body, resp, error) {
                    ...
                    if (optional == true && self.lastResponse == null) {
                        ...
                        if (self.lastResponse == null) {
                            ...
                            console.error(sh.t, '4XX-', err.stack)
                        } else {
                    ...
                    if (error) {
                        if ( error.code == 'ETIMEOUT') {
                            if ( self.showTimeOutAbbr != false  ) {

                            }
                        }
                        console.error('req Error- Error', error.message)
                        console.error(sh.t,  err.stack)
                      //X  return;
                    } else {
                        if ( resp.statusCode == 500 ) {
                            console.error('500- Error', resp.statusCoce, resp.statusMessage)
                            console.error(sh.t, '500-', err.stack)
                        }
                    }

/*
                    self.assert(resp.statusCode != 500,
                        resp.statusMessage);
                    */
            ...
            self.add(nextFunctionToAdd_getR);
            return self;
        }
        ...
        self.upload = function upload(filepath,uploadField) {
            self.nextBinding.uploadPath = filepath;
            self.nextBinding.uploadField = uploadField
            return self;
        }
