...
sh.makeDict = function makeDict(items, keyProp) {
    var dict = {};
    sh.each(items, function add(k,v){
        var keyVal = v[keyProp]
        dict[keyVal] = v;
    })
    return dict;
}

sh.concatByIndex = function concatByIndex(toArr, addItems, keyProp, skipIfNotNull) {
    skipIfNotNull = sh.dv(skipIfNotNull, true);
    var dict = sh.makeDict(toArr, keyProp);
    var keyProps = sh.makeArray(keyProp)
    function getKey(i) {
        var vals = []
        sh.each(keyProps, function addPRop(k,prop) {
            vals.push(i[prop])
        })
        var key = vals.join('_')
        return key;
    }
    sh.each(addItems, function onK(k,v) {
        var keyVal = getKey(v) //[keyProp];

        var existingVal = dict[keyVal];
        if (existingVal==null) {
            toArr.push(v)
        }
    })
...
function callIfDefined(fx,a,b) {
    var args = convertArgumentsToArray(arguments)
    var args2 = sh.args(arguments)
...
sh.logOp = sh.displayInput = function displayInput() {
    var args = sh.args(arguments)
...
sh.makeArray = function makeArray(arr) {
    if (arr == null) {
        return null;
    }
    if (arr.length) {
...
sh.str.accents = sh.str.removeAccents = sh.removeDiacritics =
...
sh.warnOnKey = function warnNOK(storeOn, prop, flagToIndicateShown, msgRest) {
    var dictWarningFlags = storeOn[prop]
    if (dictWarningFlags == null) {
        dictWarningFlags = {}
    }
    var shownB4 = dictWarningFlags[flagToIndicateShown];
    if (shownB4 !== true) {
        console.error('warning',sh.qq(flagToIndicateShown), msgRest)
    }
    dictWarningFlags[flagToIndicateShown] = true;
    storeOn[prop] = dictWarningFlags
}
...
//sh.log = {}
log.line = sh.showLine

log.lines = function createNewlines(count) {
    count = sh.dv(count, 10)
    for (var i = 0; i < count; i++) {
        console.log('\n')
    }
...
log.disable = function disableLog() {
    console.error('ignoring diable log calls')
    return;
    log.isDisabled = true;
    log.disableCount++;
    if (log.fxOld) {
...
log.showMethod = function showMethod(hop) {
    hop = sh.dv(hop, 1)
    ...
    var lines = lines.map(trim)
    var fxName = lines[1 + hop]
    fxName = sh.str.after(fxName, 'at ')
    fxName = sh.str.before(fxName, ' (')
    var line = sh.str.after(fxName, ' ')
...
function defineLogError() {
   ...
   //sh.log.error = {};
    log.error.lookFor = function lookFor(str, fx) {
        ...
        console.error = function error(d) { //
            ...
            try {
                ...
                console.error('could not post this...')
                msg = sh.args(arguments).join(' ')
            }
            if ( msg.includes(str) ) {
                fx(msg, args)
            }
    ...
    log.lookFor = function lookFor(str, fx) {

        var oldError = console.log;

        console.log = function error(d) { //
            var args = sh.args(arguments)
            var msg = sh.args(arguments).join(' ')
            try {
                msg = sh.toJ(args)
...
log.file = function file(k, msg) {
    ...
    if (msg) { console.error(sh.n, sh.t, msg) }
...
sh.TwoCallHelper = function TwoCallHelper() {
    var self = this;
    var p = this;
    p.addX = function addX(reason) {
        if (self.calledOnce === true) {
            console.error('okok', self.err.stack)
...
function defineFS() {
    ...
    sh.fs.giga = function convertMgToGigaBytes(mbs) {
       // /var gbs = mbs/1000
    ...
    sh.fs.days = sh.fs.modifiedDays = function modifiedDays(filename, dbg) {
        var stat = fs.statSync(filename)
        var fileTime = stat.mtime.getTime()
        var currentTime = (new Date()).getTime()
        var ageInMs = currentTime - fileTime;
...
function defineRemote() {
    sh.get = function getUrl(url, fxDone, qs) {
        ...
        request(req, function (error, response, body) {
            if (error) {
                console.error('error:', url, error.message); // Print the error if one occurred
            }
    ...
    sh.defineExitware = function defineExitware(app, onlyLocal) {
        ...
        console.log('exitWareWith', app.port)
...
function defineFSFunctions() {
    ...
    sh.fs.isAbs = function isAbs(file) {
        if (sh.isWin()) {
            if (file.slice(1, 2) == ':') {
                return true
            }
...
sh.textAfter = sh.str.textAfter = function getTextAfter(txt, val) {
    var t = txt.split(val)[1]
    return t;
}

sh.textBefore = sh.str.textBefore = function textBefore(txt, val) {
...
sh.throwIfNull = function throwIfNull(val, err, obj) {
    if (val != null)
        return
...
sh.removePropsFromList = function removePropsFromList(list, props) {
    var args = sh.args(arguments)
    if ( args.length > 2) {
        props = args.slice(1)
    }
    sh.each(list, function (ia, item) {
        sh.removeProps(item, props)
    })
    return list;
}
...
function defineEachMethods() {
    ...
    each.tryToCollect = function tryToCollectProp(items, propEvalStr) {
        function collectProp(item) {
            if (item == null) {
                return null;
            }
            var val = null;
            try {
                val = eval('item.' + propEvalStr)
            } catch (e) {
                val = null
            }
            return val;
        ...
        var reuslt = each.collect(items, collectProp)
        return reuslt;
    }
    ...
    each.collectKeys = each.getKeys = function getKeys(copySrc) {
        var keys = [];
        sh.each(copySrc, function addToDict(k, v) {
            keys.push(k)
            //outputList.push(newItem)
...
//AKA Lines helper
    ...
    function defineFx2() {
        ...
        each.nested = function traverseTreeOfObjects(item, fx) {
            function traverse(jsonObj) {
                if (sh.isObject(jsonObj)) {
                    sh.each(jsonObj, function (k, v) {
...
function EasyTimer() {
    ...
    this.timeElapsed = function timeElapsed(toFixed) {
        toFixed = sh.dv(toFixed, 2)
        var time = sh.timeElapsed(this.startTime).toFixed(toFixed)
        return time;
    }
    ...
    this.lap  = function howMuchTimeHasPaste(msg) {
        msg = sh.dv(msg, '')
        console.log(msg, this.secs());
        this.start()
    }
...
sh.async = function async(items, fxAction, fxDone, concurrency, syncLive) {
    var async = require('async');
    ...
    async.forEachLimit(items, concurrency, function processEachTest(item, _fxInteration) {
        //console.log('look at', item)
        itemIndex++
        function fxInteration() {
            /*if ( testDoubleCall.calledOnce ) {
             console.error('sdf',testDoubleCall.last.stack )
             console.error('sdfsdf', global.countAll1)
             console.error('sdfsdf', global.countAll2)
             console.error('sdfsdf', global.countAll3)
             console.error('sdfsdf', global.countAll4)
             debugger;
             console.error('llll')
             // return;
             }*/
    ...
    }, function allAsyncItemsProcessed(err) {
        if (err) {
            console.log(err)
        }
        ;
...
sh.time.daysPastLt = function hasXDaysPastSince(time, days) {
    var time = new Date(time)
    var currentDate = new Date()
   // var timeOk = sh.time.diff(time, currentDate, )
    var ms = sh.time.days*days
    var diff = currentDate.getTime() - time.getTime()
    if (diff < ms) { //+ ms <  ) {
        return true
    }
    return false;
...
function defineCSV() {
    sh.convertCSV = function convertCSV(contents, cfg) {
        cfg = sh.dv(cfg, {})
        var $ = sh;
...
//    }
        ...
        $.each(lines, function procLine(k, line) {
            ...
            $.each(it.columnNames, function addCol(cI, colName) {
                var fixed = colName
                colName = sh.toCamelCase(sh.unquote(fixed))
                if (cfg.noCamel) {
                    colName = (sh.unquote(fixed))
                }
                unquoted.push(colName)
            })
...
if (String.prototype.includes == null) {
    String.prototype.includes = function includes(value) {
        var found = this.indexOf(value) != -1;
        //console.log('found', found)
        return found
    }
    String.prototype.startsWith = function includes(value) {
        var found = this.indexOf(value) == 0;
        //console.log('found', found)
        return found
    }
    String.prototype.endsWith = function endsWith(value) {
        var found = this.indexOf(value) == this.length - value.length;
        //console.log('found', found)
        return found
    }
}
...
sh.mod = function isModXLog(val, mod, rest) {
    var args = sh.args(arguments)
    args = args.slice(2)
