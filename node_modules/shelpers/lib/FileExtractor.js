...
function FileExtractor() {
    ...
    function defineWork() {
        p.extractFile = function extractFile(token, cb) {
            ...
            var cmdStr = 'unzip ' +
                ['-o', sh.qq(token.fileToExtract),
                    '-d', sh.qq(dirExtraction)].join(' ');
            //silence commands
            ...
            function fxEcho(line) {
                ...
                sh.cid(self.settings.fxStatus, line + ' ' + fileName + ' ' + dirExtraction)
                console.log(line)
            ...
            sh.runAsync(cmdStr, settings.fxCallback, settings)
            //cR.execute(settings)
            //call some script
