            ...
            if (self.currentOperation == null) {
                if (self.methods.length == 0) {
                    ...
                    if (self.token.name != null) {
                        ...
                        console.log('done with promise helper'); //, self.token.name);
                    }
...
//    }
    ...
    function utilsMethods() {
        ...
        self.utils.logTest = function logTest(logMsg, addToChain) {
            var hFx = {};
            hFx.error = new Error();

            var logTextMessage = function logTextMessage(token, cb) {
                console.log()
                console.log(sh.t, '>>>>', self.token.name, logMsg)
                console.log()
                cb()
            }
            self.add(logTextMessage);
