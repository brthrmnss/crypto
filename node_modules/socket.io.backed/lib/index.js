...
Server.prototype.checkRequest = function(req, fn) {
  ...
  console.error('what is origin', '...', req.headers.origin, origin);
  if ( origin == '*' ) { //|| origin == 'http://evil.com/'){
    if ( req.headers.host   ) {
      origin = 'http://'+req.headers.host;
      console.error('hit it', origin)
      console.error(req.headers);
      if ( req.headers.referer && false ) {
        ...
        var baseOrigin = origin.split('/')[0]
        origin = protocol + baseOrigin;
  ...
  console.error('>>>skipping thing origin', origin, req.res.headers)
  //req.res.setHeader()
 // asd.f
  origin = 'http://127.0.0.1:10110/'
  req.res.setHeader("Access-Control-Allow-Origin", origin);
  req.res.setHeader("yAccess-Control-Allow-OriginX", 'checkRequest');
  ...
  if (origin) {
    try {
      ...
      //console.error('||||', parts)
      console.error('is ok', !!ok, fn.name)
      return fn(null, !!ok);
    } catch (ex) {
    }
  }
